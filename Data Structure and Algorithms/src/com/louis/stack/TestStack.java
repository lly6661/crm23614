...
public class TestStack {
    public static void main(String[] args) {
         ...
         * 就需要先对比运算符优先级，新进来的运算符如果小于等于原栈中运算符，
         * 那么需要把原运算符弹出来，数字栈中数字进行弹栈，进行运算，运算后的结果重新放入数字栈中，新运算符入栈。
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (symbolStack.isOperator(c)) {
                //如果不是一个空的符号栈，需要比较符号的优先级
                if (!symbolStack.isEmpty()) {
                    //比较运算符的优先级
                    if (symbolStack.priority(c) < symbolStack.priority(symbolStack.peak())) {
                        /**
                         * 1.去符号栈中获取栈顶的数据
                         * 2.去数字栈中获取两个数字
                         */
